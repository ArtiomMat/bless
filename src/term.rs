...
/// multi threading.
/// Also note, set_raw_input(false) is automatically called on program exit, so no need to
/// explicitely call it.
...
pub fn can_rgb() -> bool {
    unsafe {
        // XXX: I mean secure_getenv isn't necessary in fkn checking if a terminal supports RGB is
        // it?
        let name = CString::new("COLORTERM").unwrap();
        let str_ptr = libc::getenv(name.as_ptr());
